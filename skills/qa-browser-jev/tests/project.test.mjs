...
const call=(file,args,env={})=>spawnSync(process.execPath,[file,...args],{encoding:'utf8',shell:false,env:{...process.env,AI_GATEWAY_API_KEY:'',BROWSER_QA_ENV_FILE:join(tmpdir(),'qa-browser-tests-no-credentials.env'),...env}});
...
test('projetos diferentes têm configuração e hash separados', () => project(async a => project(async b => {
  await initProject(a, { url: 'http://127.0.0.1:8001', name: 'one' });
  await initProject(b, { url: 'http://127.0.0.1:8002', name: 'two' });
  const x = await readConfig(a);
  const y = await readConfig(b);
  assert.notEqual(x.configSha256, y.configSha256);
  assert.notEqual(x.config.target.baseUrl, y.config.target.baseUrl);
})));
...
test('symlink de artefatos não escapa do projeto', t => project(async a => project(async b => {
  await writeFile(join(a, 'qa.config.json'), JSON.stringify(example));
  try {
    await symlink(b, join(a, '.qa-browser-jev'), 'dir');
  } catch (error) {
    if (error.code === 'EPERM') {
      t.skip('host não permite criar symlink');
      return;
    }
    throw error;
  }
  await assert.rejects(() => createRun(a, 'QA-ONE', subject), /SYMLINK/);
})));
